...
def get_vendor_from_file(mac_address):
    tree = ET.parse(r"C:\Users\User\Talent Next\Python\OT_Project\vendormacs.xml")
    root = tree.getroot()
    mac_address = mac_address.upper()
    parts = mac_address.split(":")
    result = ":".join(parts[:3])
    for elem in root:
        if result.startswith(elem.attrib["mac_prefix"] + ":"):
...
def process_packet_mms(pkt):
    try:
        ...
        all_value=(vendor_id,product_name,revision,device_ip,device_mac)
        new_list.append(all_value)
    except AttributeError:
        pass
def get_product(pkt):
    if 'server' in pkt.browser.field_names:
        product_type=""
        if pkt.browser.server_type_workstation=='1':
            product_type=product_type+"Workstation"+","
        if pkt.browser.server_type_server=='1':
            product_type=product_type+"Server"+","
        if pkt.browser.server_type_sql=='1':
            product_type=product_type+"SQL Server"+","
        if pkt.browser.server_type_domain_controller=='1':
            product_type=product_type+"Domain Controller"+","
        if pkt.browser.server_type_backup_controller=='1':
            product_type=product_type+"Backup Controller"+","
        if pkt.browser.server_type_time=='1':
            product_type=product_type+"Time Source"+","
        if pkt.browser.server_type_apple=='1':
            product_type=product_type+"Apple Host"+","
        if pkt.browser.server_type_novell=='1':
            product_type=product_type+"Novell Server"+","
        if pkt.browser.server_type_member=='1':
            product_type=product_type+"Domain Member Server"+","
        if pkt.browser.server_type_print=='1':
            product_type=product_type+"Print Queue Server"+","
        if pkt.browser.server_type_dialin=='1':
            product_type=product_type+"Dialin Server"+","
        if pkt.browser.server_type_xenix=='1':
            product_type=product_type+"Xenix Server"+","
        if pkt.browser.server_type_ntw=='1':
            product_type=product_type+"NT Workstation"+","
        if pkt.browser.server_type_wfw=='1':
            product_type=product_type+"WFW Host"+","
        if pkt.browser.server_type_nts=='1':
            product_type=product_type+"NT Server"+","
        if pkt.browser.server_type_potential=='1':
            product_type=product_type+"Potential Browser"+","        
        if pkt.browser.server_type_backup=='1':
            product_type=product_type+"Backup Browser"+","  
        if pkt.browser.server_type_master=='1':
            product_type=product_type+"Master Browser"+","  
        if pkt.browser.server_type_domain_master=='1':
            product_type=product_type+"Domain Master Browser"+","  
        if pkt.browser.server_type_osf=='1':
            product_type=product_type+"OSF Browser"+","  
        if pkt.browser.server_type_vms=='1':
            product_type=product_type+"VMS Browser"+","  
        if pkt.browser.server_type_w95=='1':
            product_type=product_type+"WINDOWS 95 or above Host"+","  
        if pkt.browser.server_type_dfs=='1':
            product_type=product_type+"DFS Server"+"," 
        if pkt.browser.server_type_local=='1':
            product_type=product_type+"Local List only request"+"," 
        if pkt.browser.server_type_domainenum=='1':
            product_type=product_type+"Domain Enum Request"+"," 
